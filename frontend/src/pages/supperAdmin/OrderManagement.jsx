...
const OrderManagement = () => {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'admin_review': { color: 'bg-yellow-100 text-yellow-800', label: 'Pending Review' },
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Review and manage seller order requests</p>
        </div>
        <button
          onClick={() => refetch()}
          disabled={isLoading}
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-900">₹{order.totalPrice?.toLocaleString() || '0'}</div>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-md p-6 w-full max-w-md mx-4">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-600">Amount: ₹{selectedOrder.totalPrice?.toLocaleString()}</p>
            </div>
