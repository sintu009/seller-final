...
const SupplierOrders = () => {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case 'pushed': return <CheckCircle className="w-4 h-4" />;
            case 'admin_rejected': return <XCircle className="w-4 h-4" />;
    ...
    const getStatusColor = (status) => {
        switch (status) {
            case 'admin_review': return 'bg-yellow-100 text-yellow-800';
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th className="text-left py-4 px-6 font-semibold text-gray-900">Admin Notes</th>
                                </tr>
                            </thead>
                                ...
                                {filteredOrders.map((order) => (
                                    <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                                        <td className="py-4 px-6">
                                            <div className="font-mono text-sm font-medium text-emerald-600">{order.orderNumber}</div>
                                        </td>
                                        <td className="py-4 px-6">
                                            <div className="flex items-center">
                                                <User className="w-4 h-4 mr-2 text-gray-400" />
                                                <div>
                                                    <div className="font-medium text-gray-900">{order.seller?.name || 'N/A'}</div>
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="font-semibold text-emerald-600">₹{order.totalPrice?.toLocaleString() || '0'}</div>
                                        </td>
                                        <td className="py-4 px-6">
                                            ...
                                            {order.adminReview?.notes ? (
                                                <div className="text-sm text-gray-600 max-w-xs">
                                                    <div className="font-medium mb-1">
                                                        {order.adminReview.action === 'approved' ? 'Approval Notes:' : 'Rejection Reason:'}
                                                    </div>
                                                    <div className="text-xs">{order.adminReview.notes}</div>
                                                </div>
                                            ) : (
