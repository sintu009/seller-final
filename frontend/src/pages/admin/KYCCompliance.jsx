...
const KYCCompliance = () => {
  ...
  const planOptions = [
    {
      ...
      features: [
        "• Amazon account setup & GST/brand registry help",
    ...
    },
    {
      ...
      features: [
        "• 30 optimized listings + A+ content",
  ...
  const mainTabs = [
    {
      id: "seller",
      label: "Sellers",
      count: kycSubmissions.filter((k) => k.role === "seller").length,
    },
    {
      id: "supplier",
      label: "Suppliers",
      count: kycSubmissions.filter((k) => k.role === "supplier").length,
    },
  ];
  ...
  const tabs = [
    {
      ...
      count: currentSubmissions.filter((k) => k.kycStatus === "pending").length,
    },
    {
      id: "approved",
      label: "Verified",
      count: currentSubmissions.filter((k) => k.kycStatus === "approved")
        .length,
    },
    {
      id: "rejected",
      label: "Rejected",
      count: currentSubmissions.filter((k) => k.kycStatus === "rejected")
        .length,
    },
  ...
  const filteredSubmissions = kycSubmissions.filter((submission) => {
    ...
    const matchesTab =
      activeTab === "all" || submission.kycStatus === activeTab;
  ...
  const handleApprove = async (id, role) => {
    if (role === "seller") {
      ...
      console.log(
        "Approving seller KYC with plan:",
        selectedPlan,
        "and stores:",
        selectedStoresList
      );
      try {
        ...
        alert(
          `Seller KYC approved with plan: ${selectedPlan} and stores: ${selectedStoresList
            .map((s) => storeOptions.find((opt) => opt.id === s)?.name)
            .join(", ")}`
        );
        await refetch();
      } catch (error) {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Review and manage KYC documents from sellers and suppliers
            </p>
          </div>
          ...
          <div className="flex space-x-1">
            {mainTabs.map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-md font-semibold transition-colors flex items-center ${
                  mainTab === tab.id
              ...
              >
                ...
                <span
                  className={`ml-2 px-2.5 py-0.5 rounded-full text-xs font-bold ${
                    mainTab === tab.id ? "bg-orange-700" : "bg-gray-200"
                  }`}
                >
                  {tab.count}
                </span>
          ...
          {filteredSubmissions.map((submission) => (
            ...
            >
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      {submission.kycStatus === "approved" &&
                        submission.role === "seller" &&
                        submission.plan && (
                          <span
                            className={`px-3 py-1 text-sm font-medium rounded-full ${
                              submission.plan === "starter"
                                ? "bg-blue-100 text-blue-800"
                                : submission.plan === "growth"
                                ? "bg-green-100 text-green-800"
                                : submission.plan === "scale"
                                ? "bg-purple-100 text-purple-800"
                                : "bg-gray-100 text-gray-800"
                            }`}
                          >
                            <Crown className="w-3 h-3 mr-1" />
                            {submission.plan.charAt(0).toUpperCase() +
                              submission.plan.slice(1)}{" "}
                            Plan
                          </span>
                        )}
                    </div>
                ...
                <div className="flex items-center space-x-2">
                  {submission.kycStatus === "pending" && (
                    <>
                      <button
                        onClick={() =>
                          handleApprove(submission._id, submission.role)
                        }
                        disabled={
                          submission.role === "seller" &&
                          (!selectedPlans[submission._id] ||
                            !selectedStores[submission._id]?.length)
                        }
                      ...
                      <div className="flex items-center space-x-3">
                        {selectedPlans[submission._id] && (
                          <div className="flex items-center">
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-sm font-medium ${
                                selectedPlans[submission._id] === "starter"
                                  ? "bg-blue-100 text-blue-800"
                                  : selectedPlans[submission._id] === "growth"
                                  ? "bg-green-100 text-green-800"
                                  : selectedPlans[submission._id] === "scale"
                                  ? "bg-purple-100 text-purple-800"
                                  : "bg-gray-100 text-gray-800"
                              }`}
                            >
                              ...
                              {selectedPlans[submission._id]
                                .charAt(0)
                                .toUpperCase() +
                                selectedPlans[submission._id].slice(1)}{" "}
                              Plan
                            </span>
                          </div>
                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          {planOptions.map((plan) => (
                            <button
                              key={plan.id}
                              onClick={() =>
                                handlePlanSelect(submission._id, plan.id)
                              }
                              className={`p-3 rounded-md border-2 transition-all text-left ${
                                selectedPlans[submission._id] === plan.id
                                  ? "border-orange-500 bg-orange-50"
                                  : "border-gray-200 bg-white hover:border-gray-300"
                              }`}
                            >
                              <div
                                className={`inline-block px-2 py-1 rounded-full text-xs font-medium mb-2 ${plan.color}`}
                              >
                                {plan.name}
                              </div>
                          ...
                          <div className="bg-blue-50 border border-blue-200 rounded-md p-4 animate-fadeIn">
                              ...
                              {storeOptions.map((store) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    />
                                    <div
                                      className={`w-4 h-4 rounded border-2 flex items-center justify-center mr-2 ${
                                        isSelected
                            ...
                            {selectedStores[submission._id]?.length > 0 && (
                              <div className="mt-3 text-sm text-blue-700">
                                Selected:{" "}
                                {selectedStores[submission._id]
                                  .map(
                                    (id) =>
                                      storeOptions.find((s) => s.id === id)
                                        ?.name
                                  )
                                  .join(", ")}
                              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium text-gray-900">
                      {submission.kycDocuments?.businessName ||
                  ...
                  <div>
                    ...
                    <div className="font-mono text-gray-900">
                      {submission.kycDocuments?.businessRegistration ||
              ...
              {submission.kycDocuments &&
                Object.keys(submission.kycDocuments).length > 0 && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      {submission.kycDocuments?.bankDetails
                        ?.cancelledChequePath && (
                        <div className="border border-gray-200 rounded-md p-4">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center">
